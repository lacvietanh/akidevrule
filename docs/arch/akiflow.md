...
### The gate had only one answer for "no": leave

Condition 2 is a good filter and a bad exit. Work that fails it — a sweep, a migration, a fan-out whose answer is already knowable — was told to route itself to bare workers or to the native `Workflow` tool, which meant giving up the anchor, the quoted requirements, the `[RULES]` receipts, the durable on-disk record and the closure gate along with the debate it genuinely did not need. Those two things were bundled for no reason other than that only one shape had ever been built.

The evidence that this was costing something is in the rooms themselves. Of 70 live sessions, **19 posted no debate turn at all, and 11 of those still did substantive work** — checklists of 4 to 16 KB, one room with 14 seat files — while 13 more posted between one and five turns. They were fan-outs, executed inside council scaffolding, paying for machinery they never used (full measurement trail: `docs/research/akiflow-cost-and-shape-aug15.md`). `--convene` compounded it by refusing to open any room whose items named no challenger, so the correct move for better than a quarter of all sessions was to abandon the skill.

**Shape is therefore a question prior to the gate: is anything being arbitrated?** If two competent seats could reach different defensible answers, it is a council. If the answer is knowable and the work is merely large, it is a dispatch. Shape and mode are independent axes — a dispatch can be `audit` or `execute` exactly as a council can.

A dispatch partitions into **lanes with an exclusive set of files each may write**, rather than items with an adversary, because the two shapes die of different things. A council's failure is a decision nobody attacked; a fan-out's failure is two workers editing one file, which no amount of judgment prevents and which stays invisible until the second write clobbers the first. So `--convene` in dispatch mode checks something a council never needed to: that no path appears in two lanes' `writes:`. The three-condition gate is replaced by a different and equally falsifiable one — partitionable into ≥2 independent lanes, each lane's paths and question nameable up front, and a result that must outlive the session. Fail the last and a bare spawn suffices; fail the middle one and it was a council question after all.

Dispatch is a mode of the same skill rather than a second skill, and reuses the same workspace, the same three artifacts and all seven checks of `council_verify.py` unchanged. The lane itself is the declared seat exactly as a council's `owner` is — its `worker` is substrate/cost metadata, not the trace identity, since two lanes can legitimately share a worker type and must still trace separately; an untraced lane is a ghost by the same check. The one defense genuinely surrendered is self-approval — there is no adversary — and the honest statement of the tradeoff is that wanting one is the signal that the wrong shape was picked. Correctness in a dispatch has to be checkable by reading or by machine; where it is only checkable by argument, reopen as a council.
...
Four consequences follow from the cost model in `references/harness-facts.md`, and each contradicts an intuition that would otherwise go unchallenged:
...
- **A read is a subscription, not a purchase — and this is the lead's dominant cost.** Every turn re-sends the whole history, so anything pulled into context is charged again on each later turn: a read of size `S` at turn `t` of a `T`-turn run costs roughly `S × (T − t)`. Pulling a 50k-token room at turn 50 of 200 is ~7.5M cache-read tokens from a single call. Measured on a real run of this skill's own repair, with three `aki-maker` workers doing every file edit, the lead still held **70% of cache-read and 66% of output** — and it held 76% / 72% at an earlier point in the same run, against 63% / 65% on the run measured 2026-08-12. Delegation had moved the work but not the money: what the run pays for is the lead's own accumulated context, not the size of the roster. This is the reason `council_read.py` grew `--grep` and `--turn`: locate for a few hundred bytes, then pull one turn (measurements and method: `docs/research/akiflow-cost-and-shape-aug15.md`). Every other slicing flag needs you to already know where to look, which is why the transcript corpus holds 110 whole-file reads of a `chat.md` against 10 sliced ones from before the search existed.
...
**The tally is complete over the meter it exists to police, and stops there deliberately.** The harness writes every turn's `model` and `usage` for the lead and for each subagent it spawned, so `council_cost.py` returns the exact Claude-side figure rather than a sample of it — which is the whole of what Step 1's roster declaration promised, since a roster is declared in Claude models. A lane run through agy headless or a separate `claude -p` process is not summed in, and that is the design rather than a gap: those run against a different vendor's quota (see § A second axis: which vendor pays), so a combined figure would be a total in no single currency — arithmetic that obscures the one number being reconciled instead of extending it. Where an off-quota lane's own spend matters, it is read from that call's `usage` and reported *beside* the table, never folded into it. `--max-budget-usd` is the preventive complement, not a substitute — a dollar cap set before the run, next to a reconciliation done after it.
...
**The gate stopped requiring the seat, and that is the whole lesson.** The previous `council_verify.py` failed any session where `akirule-enforcer` had not posted — so the script *forced* the seat to exist regardless of whether anything needed enforcing, and when it fired on a run with nothing to enforce, the lead renamed a chat header to make it pass rather than concluding the seat did not belong. **A gate that manufactures work gets gamed rather than questioned.** Roster composition is judgment; presence of evidence is not. The current gate therefore checks seven things and names no seat: a non-empty anchor block · every `REQ-<n>` quoting a fragment found in it · no ghost seats (a declared owner/challenger that left neither a turn nor a file) · a `[RULES]` receipt from every seat · per-seat evidence tags · every `REMIND-<n>` answered by an `ACK` or a logged `OVERRULE` · every ledger `REQ-<n>` named by some item's `covers`.

**A seat's evidence is its turns plus its own file, and a check with nothing to check says so.** Checks 3-5 originally read `chat.md` alone and iterated the posting agents, which was wrong in both directions once the corpus was measured: 48 of 70 live rooms keep `<seat>.md` files the gate never opened, so a seat that worked in its file was called a ghost — and 19 rooms have no `### ` turns at all, so the receipt and tag checks iterated an empty list and printed PASS, scoring a room they had not looked at. The roster is now `declared ∪ posters`, a seat file supplies evidence only for a name already on it, and a check whose subject set is empty prints `SKIP`. The distinction is the point: a mini room whose hands are external CLIs seats no Claude agent legitimately, and reporting "passed" about it is the same class of lie the gate was built to catch.
...
12. A seat's declared tier and its executing tier disagreeing → the roster line says one thing and the harness does another, in either direction. Omitting `model` does **not** fall through to the lead: the Agent tool reads the agent definition's frontmatter first and inherits the caller's tier only where the definition names none, so the expensive case is a **generic** seat (`general-purpose`) spawned tier-less — which is exactly Step 6's cost seat, since it must hold `Bash`. The opposite case is quieter and now the live one: a frontmatter tier that under-serves what the roster guidance asks of that seat, e.g. an adversarial reviewer pinned to a mid tier while the guidance calls for a strong model. Frontmatter is the single source of truth for `model`; a spawn passes it only to override. (`effort` is a headless-only flag; the in-session Agent tool has no such parameter, verified 2026-08-03.)
...
20. A close-out tally that does not name the meter it covers → `council_cost.py` measures the Claude session and every subagent under it, exactly and completely; a lane on another vendor's quota is a different currency, reported beside that number and never summed into it. An unlabelled total reads as the run's whole cost and is not.
21. A ghost roster seat → declared at the gate, cited as challenger, leaves neither a turn nor a file; items close against a challenge that never happened — the run's only audit removed with no error. Caught mechanically by `council_verify.py`.
